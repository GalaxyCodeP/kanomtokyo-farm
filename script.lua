...
-- ตัวแปรสำหรับเช็คว่า script ถูกรันครั้งแรกหรือไม่
...
local function Log(message, level)
    ...
    print(logMessage)
    warn(logMessage) -- แสดงใน output เป็นสีเหลือง
end

Log("Starting Auto Farm Script...")

-- ลบ Platform เก่าทั้งหมดที่ค้างอยู่ใน workspace
...
Log("Loading Rayfield UI Library...")
local Rayfield
...
if not success or not Rayfield then
    Log("Failed to load Rayfield: " .. tostring(err), "ERROR")
    return
end
Log("Rayfield loaded successfully!")

-- Create Window
Log("Creating Window...")
local Window = Rayfield:CreateWindow({
    Name = "Auto Farm by n_nntd (AKIHUB)",
    LoadingTitle = "Loading...",
    LoadingSubtitle = "by n_nntd",
    ConfigurationSaving = {
        Enabled = false,
    },
    Discord = {
...
-- Create Section
Log("Creating Farm Section...")
local FarmSection = MainTab:CreateSection("Auto Farm Settings")
Log("Farm Section created successfully!")
...
-- ฟังก์ชันตรวจสอบว่าผู้เล่นตายหรือไม่
...
-- ระบบตรวจจับการตายและกด E อัตโนมัติ
task.spawn(function()
    ...
    while task.wait(0.5) do
        if _G.AutoFarm then
            if IsPlayerDead() then
                ...
                -- ตรวจสอบว่า AutoFarm ยังเปิดอยู่หรือไม่
                if _G.AutoFarm then
                    PressE()
    ...
    repeat
        ...
        for _, v in ipairs(Player.PlayerGui:GetDescendants()) do
            ...
            if (v:IsA("TextButton") or v:IsA("ImageButton")) and v.Visible then
                ...
                if (label and label.Text:find("Understood"))
                or (v:IsA("TextButton") and v.Text:find("Understood")) then
...
local function CreatePlatform()
    if not platformPart or not platformPart.Parent then
        ...
        platformPart.Transparency = 1 -- ล่องหนสมบูรณ์
...
local function TweenToEnemy(enemyHRP)
    ...
    -- ถ้าอยู่ใกล้เป้าหมายแล้ว (น้อยกว่า 3 studs) ให้หยุดอยู่นิ่งๆ ไม่ต้อง tween
    if dist < 3 then
        -- ล็อคตำแหน่งให้อยู่นิ่ง
    ...
    -- ทำให้ทะลุ Part ได้ (ยกเว้น Platform)
    ...
    -- ยกเลิก tween เก่าถ้ามี
    if currentTween then
        currentTween:Cancel()
    end
    
    -- วาง Platform ใต้ตัวละคร
    platform.CFrame = CFrame.new(targetPos.X, targetPos.Y - 3, targetPos.Z)
...
-- ฟังก์ชันให้ตัวละครอยู่บน Platform เมื่อหาเป้าหมายไม่เจอ
...
task.spawn(function()
    ...
    while task.wait(0.25) do
        ...
        if not questName or not enemyNames then 
            ...
            StayOnPlatform() -- อยู่บน Platform ไว้
            continue 
        end
        ...
        if type(enemyNames) == "string" and enemyNames ~= "" then
            for name in string.gmatch(enemyNames, "[^,]+") do
                if type(name) == "string" then
                    ...
                    if cleanName ~= "" then
                        table.insert(enemiesToFarm, cleanName)
                    end
                end
            end
        end
        ...
        for _, enemyName in ipairs(enemiesToFarm) do
            ...
            if enemy then
                ...
                break -- ออกจาก loop หลังจากตีมอนตัวนี้เสร็จ
            end
        end
        
        -- ถ้าหาเป้าหมายไม่เจอเลย ให้อยู่บน Platform ไว้
...
local AutoFarmToggle = MainTab:CreateToggle({
    ...
    Callback = function(Value)
        _G.AutoFarm = Value
        Log("Auto Farm: " .. tostring(Value))
        
        -- กด E อัตโนมัติเมื่อเปิด Auto Farm ครั้งแรก
        if Value and not hasInitialized then
            hasInitialized = true
            Log("First time enabling Auto Farm - pressing E to equip weapon...")
            task.wait(0.5)
            PressE()
        end
    end,
...
Log("Creating Dropdown: Fly Position...")
local PositionDropdown = MainTab:CreateDropdown({
    Name = "Fly Position",
    Options = {"Above", "Below"},
    CurrentOption = "Below",
    ...
    Callback = function(Option)
        -- แก้ไขการรับค่า Option
        local selectedOption = Option
        if type(Option) == "table" then
            selectedOption = Option[1] or "Below"
        end
        _G.FlyPosition = selectedOption
        Log("Fly Position changed to: " .. tostring(selectedOption))
    end,
})
Log("Dropdown created!")

Log("Creating Slider: Distance...")
local DistanceSlider = MainTab:CreateSlider({
    ...
    Flag = "Distance",
    Callback = function(Value)
        _G.DistanceFromEnemy = Value
        Log("Distance: " .. tostring(Value))
    end,
})
Log("Slider created!")

Log("Creating Slider: Tween Speed...")
local SpeedSlider = MainTab:CreateSlider({
    ...
    Callback = function(Value)
        _G.TweenSpeed = Value
        Log("Tween Speed: " .. tostring(Value))
    end,
})
Log("Slider created!")

Log("Creating Label: Status...")
